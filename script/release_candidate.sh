...
DIRECT_DMG="$OUTPUT_DIR/Sorting-Hat-v$VERSION.dmg"
...
DMG_SHA="$(shasum -a 256 "$DIRECT_DMG" | awk '{print $1}')"
...
printf '%s\n' \
  ...
  "developer_id_dmg=$DIRECT_DMG" \
  "developer_id_dmg_sha256=$DMG_SHA" \
  "sparkle_appcast=$APPCAST" \
  "sparkle_appcast_sha256=$APPCAST_SHA" \
