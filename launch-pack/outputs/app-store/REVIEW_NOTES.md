...
On a supported Mac running macOS 26 with Apple Intelligence enabled, choose the Apple provider and “On this Mac” to exercise local generation. On other supported Macs, Ollama is optional and must run on the same Mac at `localhost`, `127.0.0.1`, or `::1`. OpenAI and remote Ollama are unavailable in this build.
...
The app does not require a network connection when Apple's on-device provider is available and selected. Local Ollama settings may be left blank.
